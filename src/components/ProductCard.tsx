...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="product-card group rounded-[1.75rem] bg-white p-3 shadow-[0_1px_0_rgba(0,0,0,0.06)] transition duration-500 hover:-translate-y-1 hover:shadow-soft">
      <Link href={`/shop/${product.slug}`} className="block focus-ring rounded-[1.25rem]">
        <div className="relative aspect-[4/5] overflow-hidden rounded-[1.25rem] bg-circle-cream">
          ...
          <div className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-black backdrop-blur">
            {product.priceBand}
          </div>
      ...
      <div className="px-2 pb-2 pt-4">
        <div className="flex items-baseline justify-between gap-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-black/45">{product.category}</p>
          <p className="text-[11px] font-semibold text-black/45">MOQ {product.moq}</p>
        </div>
        <h3 className="mt-1 font-display text-xl text-black">{product.name}</h3>
        <p className="mt-2 min-h-[42px] text-sm leading-6 text-black/58">{product.description}</p>
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="focus-ring min-h-11 rounded-full border border-black/10 px-4 text-xs font-bold uppercase tracking-[0.14em] transition hover:bg-black hover:text-white"
          >
            Add to inquiry
          </button>
        </div>
